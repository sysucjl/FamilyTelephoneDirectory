...
import com.example.sysucjl.familytelephonedirectory.tools.AvatarDataHelper;
...
public class EditActivity extends AppCompatActivity{
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if(mTag.equals(TAG_EDIT)){
            ...
            if(mContactAvatar != null){
                ivAvatar.setMinimumHeight(ScreenUtils.getScreenSize(this).x);
                Picasso.with(this)
                        .load(mContactAvatar)
                        .into(ivAvatar);
            }
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        switch (requestCode){
            case PHOTO_REQUEST:
                ArrayList<String> imagePaths = data.getStringArrayListExtra(MultiImageSelectorActivity.EXTRA_RESULT);
    ...
    private void startPhotoZoom(Uri data) {
        ...
        intent.putExtra("outputY", point.x);
    ...
    private void savePhoneNum(String s, long rawContactId, int position) {
        ...
        values.put(ContactsContract.CommonDataKinds.Phone.NUMBER,
                s);
        String phoneType = PHONETYPELIST.get(position);
        values.put(ContactsContract.CommonDataKinds.Phone.TYPE, PHONETYPE.get(phoneType));
        getContentResolver().insert(android.provider.ContactsContract.Data.CONTENT_URI,
                values);
        values.clear();
        System.out.println("存储号码："+s+" "+phoneType);
    }
